...
export function BlogCard({ blog }: BlogCardProps) {
  return (
    <Link href={`/blogs/${blog.slug}`} className="group block">
      <Card className=" h-[424px] p-3 bg-background border border-muted/20 rounded-lg group-hover:shadow-lg transition-all duration-300 hover:scale-[1.02] overflow-hidden">
        {/* Header Image */}
        <div className="relative rounded-t-lg h-[200px] bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center p-2">
          {blog.image_url ? (
            <Image
              src={blog.image_url || ""}
              alt={blog.title}
        ...
        <CardHeader className="pb-3">
          <div className="flex justify-between items-start mb-3">
            <Badge
              variant="outline"
              className="text-xs font-medium px-2 py-1 border-blue-200 text-blue-600 bg-blue-50" style={{ fontWeight: '400' }}
            >
        ...
        <CardContent className="pt-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-500 font-ibm-plex" style={{ fontWeight: '400' }}>
